...
class _ProfilEditScreenState extends State<ProfilEditScreen> {
  ...
  void initState() {
    super.initState();
    nama_usahaController = TextEditingController(
      text: widget.profile.nama_usaha,
    );
  ...
  Future<void> _save() async {
    ...
    if (emailController.text != widget.profile.email) {
      final existingProfile = await profileProvider.getProfileByEmail(
    ...
    final updatedProfile = widget.profile.copyWith(
      ...
      foto_profil: widget.profile.foto_profil,
    );
  ...
  Future<void> pickAndUpdatePhoto(int id_user) async {
    ...
    if (filePath.length > 150) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Foto berhasil diupdate')));
  ...
  Widget build(BuildContext context) {
    ...
    final profile = profileProvider.profile ?? widget.profile;
    return Scaffold(
            ...
            CircleAvatar(
              ...
              backgroundImage:
                  (profile.foto_profil != null &&
                          profile.foto_profil.isNotEmpty)
                      ? FileImage(File(profile.foto_profil))
                      : const AssetImage('assets/icons/default-user-image.png')
                          as ImageProvider,
            ),
            SizedBox(height: 30),
            _textField(
              ...
              onChanged: (value) {
                if (value.length > 50) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        'Nama usaha tidak boleh lebih dari 50 karakter',
                      ),
                    ),
                  );
                }
            ...
            _textField(
              ...
              onChanged: (value) {
                if (value.length > 20) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('No HP tidak boleh lebih dari 20 karakter'),
                    ),
                  );
                }
              },
            ...
            _textField(
              ...
              onChanged: (value) {
                if (value.length > 50) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('Email tidak boleh lebih dari 50 karakter'),
                    ),
                  );
                } else if (!RegExp(
